...
{
	struct rusage ru;
	struct uftrace_proc_page_fault *page_fault = buf;
		       ...
		       enum trigger_read_type type)
{
	...
	if (type & TRIGGER_READ_PROC_PAGE_FAULT) {
		struct mcount_event *event;

		if (mtdp->nr_events < MAX_EVENT) {
			event = &mtdp->event[mtdp->nr_events++];

			event->id    = EVENT_ID_PROC_PAGE_FAULT;
			event->time  = rstack->start_time;
			event->dsize = sizeof(struct uftrace_proc_page_fault);
